def bubble(list_a):
    indexing_length = len(list_a) - 1  # 2  #SCan not apply comparision starting with last item of list (No item to right)
    ...
    while not sorted:  #Repeat until sorted = True
        ...
        for i in range(0, indexing_length): # For every value in the list
            if list_a[i] > list_a[i + 1]: #"Angled brackets not allowed in Youtube Description :
